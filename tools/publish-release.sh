...
if [ "$UPLOAD" == "true" ]; then
    ...
    stage "Uploading the bundle to CDN: $BUNDLE_TAR -> ftp://uploadcds.labs.intellij.net"
    curl --upload-file "$BUNDLE_TAR" "ftp://$CDN_USER:$CDN_PASS@uploadcds.labs.intellij.net"
    echo "Available at https://download.jetbrains.com/kotlin/native/releases/$OS/$BUNDLE_TAR"
